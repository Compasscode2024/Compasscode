...
static const int fairness_exp = 1;
namespace ns3 {
...
void SwitchNode::SendToDev(Ptr<Packet>p, CustomHeader &ch){
	if (fairness_exp) {
		if (ch.udp.dport > 100) {
			int id = (int)(ch.udp.dport) - 100;
			uint64_t start = 2000000000 + id * 10000000;// 2.01, 2.02, 2.03, 2.04
			uint64_t end = 2090000000 - id * 10000000;// 2.08, 2.07, 2.06, 2.05
			// uint64_t start = 2000000000LL + id * 100000000;// 2.1, 2.2, 2.3, 2.4
			// uint64_t end = 2900000000LL - id * 100000000;// 2.8, 2.7, 2.6, 2.5
			uint64_t cur = Simulator::Now().GetTimeStep();
			if (1 <= id && id <= 4 && cur > end) {
				return;
			}
		}
	}
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p){
	...
	uint64_t used_qlen_scale = qlen_scale/(dev1->GetDataRate().GetBitRate()/100000000000);
	...
	uint64_t now_t = Simulator::Now().GetTimeStep();
	uint32_t qlen = dev1->GetQueue()->GetNBytesTotal();
	if (1){
		...
		if (buf[PppHeader::GetStaticSize() + 9] == 0x11){ // udp packet
			...
			if (m_ccMode == 3 || m_ccMode == 11){ // HPCC
				...
				ih->PushHop(Simulator::Now().GetTimeStep(), m_txBytes[ifIndex], qlen, dev->GetDataRate().GetBitRate());
	...
	uint64_t pst_qdelay;
	uint64_t pst_qdelay_time;
	if (now_t - used_qlen_scale< 2000000000)
	{

		pst_qdelay = 0;
		pst_qdelay_time = now_t - used_qlen_scale;
	}
	else
	{

		pst_delay_index = (now_t - used_qlen_scale - 2000000000) /used_delay_scale;
		pst_delay_index = pst_delay_index % delay_scale_mon;
		pst_qdelay = m_delay[ifIndex][pst_delay_index];
		pst_qdelay_time = m_delay_time[ifIndex][pst_delay_index];
		if (now_t - pst_qdelay_time > m_maxRtt)
		pst_qdelay_time = now_t - used_qlen_scale;
	}

	uint32_t cur_delay_index = (now_t - 2000000000) / used_delay_scale;
	cur_delay_index = cur_delay_index %delay_scale_mon;
	m_delay[ifIndex][cur_delay_index] = qlen;
	m_delay_time[ifIndex][cur_delay_index] = now_t;
	

	double qleng = ((double)qlen - (double)pst_qdelay)/((double)(now_t) -(double)pst_qdelay_time)* 1e9 * 8;
	double thresh = 0.05 * m_maxRtt * dev1->GetDataRate().GetBitRate()/8/1e9;
	if (qlen > 0)
	{
		uint64_t iinc = 0;
		double power_fact =m_first_RTT_rate[ifIndex]/ (dev1->GetDataRate().GetBitRate() - m_first_RTT_rate[ifIndex] ) * (qleng)/dev1->GetDataRate().GetBitRate();
		
		double abs_fact = power_fact;
		if (power_fact < 0)
		abs_fact = -power_fact;
		iinc = abs_fact * (double) (p->GetSize());
		...
		if (power_fact > 0)
		m_txBytes[ifIndex] += iinc;
		else
		m_txBytes[ifIndex] -= iinc;
	}
	CustomHeader ch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
	p->PeekHeader(ch);
	if (fairness_exp && m_id == 64 && ifIndex == 3 && ch.udp.dport > 100)
	{	int fid = 0;
		fid = ch.udp.dport - 100;
		uint64_t dt = Simulator::Now().GetTimeStep() - m_lastPktTs1[fid];
		if (dt > tx_scale)
		{
			double txRate = double(m_lastPktSize[fid]) / double(dt) * 8 * 1e9;
			m_lastPktSize[fid] = 0;
			m_lastPktTs1[fid] = Simulator::Now().GetTimeStep();
			// if (m_id == 64 && ifIndex == 3)
			// if ( Simulator::Now().GetTimeStep() >= 2001013031 && Simulator::Now().GetTimeStep() < 2001013031 + 2000000)
			 std::cout<<(double)(Simulator::Now().GetTimeStep()- 0)/(double)1e6<<" "<<fid<<" "<<txRate/1e9<<std::endl;
		}
		m_lastPktSize[fid] += p->GetSize(); 
	}
	else
	{
