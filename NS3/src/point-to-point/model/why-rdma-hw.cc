...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp){
	...
	if (m_mtu < payload_size)
		payload_size = m_mtu;
	// printf("payload_size = %d, qp->GetBytesLeft = %d\n", payload_size, qp->GetBytesLeft());
...
void RdmaHw::UpdateRateHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react){
	...
	if (qp->hp.m_lastUpdateSeq == 0){ // first RTT
		...
		if (ih.nhop <= IntHeader::maxHop){
			...
			if (ch.dip == 184550145)  	 
				fout<<U<<" "<<new_rate<< " " << qp->hp.m_curRate << " "<< qp->hp.u<<" "<<Simulator::Now().GetTimeStep()<<std::endl;
			if (updated_any)
...
void RdmaHw::HandleAckDctcp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	...
	if (cnp && qp->dctcp.m_caState == 0){
		// printf("RdmaHw::HandleACKDctcp::hey we are here\n");
		#if PRINT_LOG
		...
		printf("%.3lf\n", qp->m_rate.GetBitRate() * 1e-9);
		#endif
		// printf("!!%.3lf, %.3lf\n", m_minRate.GetBitRate() * 1e-9, qp->m_max_rate.GetBitRate() * 1e-9);
		// printf("$$%.3lf\n", qp->m_rate.GetBitRate() * 1e-9);
	...
	// printf("qp->mcaState = %d, newbatch = %d, dctcprai = %lld\n", qp->dctcp.m_caState, new_batch, m_dctcp_rai.GetBitRate());
...
// double RdmaHw::NormPower(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
// 	IntHeader &ih = ch.ack.ih;

// 	if (qp->hp.m_lastUpdateSeq == 0) {
// 		qp->hp.m_lastUpdateSeq = qp->snd_nxt;
// 		for (uint32_t i = 0; i < ih.nhop; i++) {
// 			qp->hp.hop[i] = ih.hop[i];
// 		}
// 		return 1;
...
// 	double tau = qp->m_baseRtt * 1e-9;// rtt in seconds

// 	for (uint32_t i = 0; i < ih.nhop; i++){
		
// 		double dt = ih.hop[i].GetTimeDelta(qp->hp.hop[i]) * 1e-9;// dt in seconds
// 		// if (dt > 0.00001) {
// 		// 	printf("ih.hop[i].ts = %lld, qp->hop[i].ts = %lld\n", ih.hop[i].time, qp->hp.hop[i].time);
// 		// 	exit(0);
// 		// }

// 		double q = ((int)ih.hop[i].GetQlen() - (int)qp->hp.hop[i].GetQlen()) * 8 / dt;// dq/dt
// 		double mu = (ih.hop[i].GetBytesDelta(qp->hp.hop[i])) * 8 / dt;// txRate
// 		double lambda = q + mu;// current

// 		double BDP = ih.hop[i].GetLineRate() * tau;

// 		double v = ih.hop[i].GetQlen() * 8 + BDP;// voltage
// 		double Gamma_tmp = lambda * v;// tmp power

// 		double e = tau * ih.hop[i].GetLineRate() * ih.hop[i].GetLineRate();
// 		double Gamma_tmp_norm = Gamma_tmp / e;// normalized tmp power

// 		// if (Gamma_tmp_norm > 2) {
// 		// 	printf("%f\n", Gamma_tmp_norm);
// 		// }
// 		// NS_ASSERT(Gamma_tmp_norm <= 2);

// 		if (Gamma_tmp_norm > Gamma_norm) {
// 			Gamma_norm = Gamma_tmp_norm;
// 			deltat = dt;
// 		}
...
// 		for (uint32_t i = 0; i < ih.nhop; i++) {
// 			printf("----- i = %d, nhop = %d -----\n", i, ih.nhop);
// 			printf("linerate = %lld, tau = %.10f\n", ih.hop[i].GetLineRate(), tau);
// 			double dt = ih.hop[i].GetTimeDelta(qp->hp.hop[i]) * 1e-9;// dt in seconds
// 			printf("dt = %.10f\n", dt);
// 			if (dt > 0.00001) {
// 				printf("ih.hop[i].ts = %lld, qp->hop[i].ts = %lld\n", ih.hop[i].time, qp->hp.hop[i].time);
...
// 			}
// 			double q = ((int)ih.hop[i].GetQlen() - (int)qp->hp.hop[i].GetQlen()) * 8 / dt;// dq/dt
// 			printf("ih.hop.qlen = %d, qp->hop.qlen = %d\n", ih.hop[i].GetQlen(), qp->hp.hop[i].GetQlen());
// 			printf("q = %.10f\n", q);
			
// 			double mu = (ih.hop[i].GetBytesDelta(qp->hp.hop[i])) * 8 / dt;// txRate
// 			double lambda = q + mu;// current
// 			printf("ih.hop.bytes = %d, qp->hop.bytes = %d\n", ih.hop[i].GetBytes(), qp->hp.hop[i].GetBytes());
// 			printf("mu = %.10f, lambda = %.10f\n", mu, lambda);
...
// 			printf("v = %.10f, Gamma_tmp = %.10f\n", v, Gamma_tmp);

// 			double e = tau * ih.hop[i].GetLineRate() * ih.hop[i].GetLineRate();
// 			double Gamma_tmp_norm = Gamma_tmp / e;// normalized tmp power
// 			printf("e = %.10f, Gamma_tmp_norm = %.10f\n", e, Gamma_tmp_norm);
...
// 	// printf("tau = %.10f, deltat = %.10f, Gamma_norm = %f\n", tau, deltat, Gamma_norm);
// 	qp->powertcp.Gamma_smooth = (qp->powertcp.Gamma_smooth * (tau - deltat) + Gamma_norm * deltat) / tau;

// 	return qp->powertcp.Gamma_smooth;

// 	// static double Gamma_smooth = 1;
// 	// Gamma_smooth = (Gamma_smooth * (tau - deltat) + Gamma_norm * deltat) / tau;
...
// void RdmaHw::HandleAckPowertcp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
// 	uint32_t next_seq = qp->snd_nxt;

// 	double normPower = NormPower(qp, p, ch);


// 	static const double gamma = 0.9;

// 	double rtt = qp->m_baseRtt * 1e-9;
// 	DataRate cwnd = qp->m_rate * rtt;
// 	DataRate beta = m_rai * rtt;

// 	// printf("cwnd_old = %f\n", cwnd);

// 	// printf("cwnd = %lld, normPower = %f, cwnd = %lld\n", cwnd.GetBitRate(), normPower, (cwnd / normPower).GetBitRate());
// 	cwnd = gamma * (cwnd / normPower + beta) + (1 - gamma) * cwnd;
// 	// printf("gamma = %f, normPower = %f, cwnd = %lld, beta = %lld\n", gamma, normPower, cwnd.GetBitRate(), beta.GetBitRate());
// 	// printf("m_rai = %lld, rtt = %f, beta = %lld\n", m_rai.GetBitRate(), rtt, beta.GetBitRate());
// 	DataRate new_rate = cwnd / rtt;
// 	// printf("new_rate = %lld, minrate = %lld\n", new_rate.GetBitRate(), m_minRate.GetBitRate());
...
// 	// 	printf("rate = %lld\n", qp->m_rate.GetBitRate());
// }
void RdmaHw::HandleAckPw(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	uint32_t ack_seq = ch.ack.seq;
	// update rate
	if (ack_seq > qp->hp.m_lastUpdateSeq){ // if full RTT feedback is ready, do full update
		UpdateRatePw(qp, p, ch, false);
	}else{ // do fast react
		FastReactPw(qp, p, ch);
	}
}

void RdmaHw::UpdateRatePw(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react){
	uint32_t next_seq = qp->snd_nxt;
	bool print = !fast_react || true;
	if (qp->hp.m_lastUpdateSeq == 0){ // first RTT
		qp->hp.m_lastUpdateSeq = next_seq;
		...
		if (ih.nhop <= IntHeader::maxHop){
			...
			for (uint32_t i = 0; i < ih.nhop; i++){
				...
				double duration = tau * 1e-9;
				// printf("duration = %.10f\n", duration);
				double q = ((int)ih.hop[i].GetQlen() - (int)qp->hp.hop[i].GetQlen()) * 8 / duration;
				double txRate = (ih.hop[i].GetBytesDelta(qp->hp.hop[i])) * 8 / duration;
				//std::cout<<ih.hop[i].GetQlen()<<std::endl;
				double lambda = q + txRate;
				//if (ch.dip == 184550145)
				//fout<<ih.hop[i].GetBytesDelta(qp->hp.hop[i])<<std::endl;
					//fout<<(int)ih.hop[i].GetQlen()<<" "<<q<<" "<<txRate<<" ";
				uint64_t b = ih.hop[i].GetLineRate();
				double rtt = qp->m_baseRtt * 1e-9;
				double BDP = (double)b * rtt;

				double v = ih.hop[i].GetQlen() * 8 + BDP;
				if (lambda < 0)
				lambda = 0;
				double Gamma_tmp = lambda * v;

				double e = rtt * b * b;
				double Gamma_tmp_norm = Gamma_tmp / e;

				double u = Gamma_tmp_norm;
			...
			if (!m_multipleRate){
				...
				if (updated_any){
					...
					if (max_c >= 1 || qp->hp.m_incStage >= m_miThresh){
						new_rate = gamma * (qp->hp.m_curRate / max_c + m_rai) + (1 - gamma) * qp->m_rate;
						new_incStage = 0;
					}else{
						new_rate = gamma * (qp->hp.m_curRate + m_rai) + (1 - gamma) * qp->m_rate;
						new_incStage = qp->hp.m_incStage+1;
				...
				for (uint32_t i = 0; i < ih.nhop; i++){
					if (updated[i]){
						...
						double c = qp->hp.hopState[i].u / m_targetUtil;
						if (c >= 1 || qp->hp.hopState[i].incStage >= m_miThresh){
							new_rate_per_hop[i] = gamma * (qp->hp.hopState[i].Rc / c + m_rai) + (1 - gamma) * qp->m_rate;
							new_incStage_per_hop[i] = 0;
						}else{
							new_rate_per_hop[i] = qp->hp.hopState[i].Rc + m_rai;
							new_incStage_per_hop[i] = qp->hp.hopState[i].incStage+1;
			...
			//if (ch.dip == 184550145)
			  	 //fout<<U<<" "<<qp->hp.u<<" "<<new_rate<< " " << Simulator::Now().GetTimeStep()<<std::endl;
			if (updated_any)
				ChangeRate(qp, new_rate);
			if (!fast_react){
				if (updated_any){
...
void RdmaHw::FastReactPw(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	if (m_fast_react)
		UpdateRatePw(qp, p, ch, true);
}
