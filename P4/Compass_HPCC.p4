...
struct my_egress_metadata_t {
    ...
    bit<8> QL_exp;
    bit<32> past_QD;
    bit<4> factor_1_mantissa; //B/(B-R)
    bit<4> factor_2_mantissa; //(RB)/(B-R)
}
    ...
    inout egress_intrinsic_metadata_for_output_port_t  eg_oport_md)
{
    ...
    RegisterAction<bit<32>, bit<16>, bit<32>>(QD_odd_reg) operate_QD_odd_reg =
    {
        void apply(inout bit<32> register_data, out bit<32> result) {
            result = register_data;
            if (meta.odd_or_even == 1)
            {
                register_data = (bit<32>) eg_intr_md.deq_timedelta;
            }
    ...
    RegisterAction<bit<32>, bit<16>, bit<32>>(QD_even_reg) operate_QD_even_reg =
    {
        void apply(inout bit<32> register_data, out bit<32> result) {
            result = register_data;
    ...
    action update_QD_even_a()
    {   
        operate_QD_even_reg.execute(meta.update_index);
    }
    @stage(4)  table update_QD_even_t
    {   
        actions={update_QD_even_a;}
        default_action=update_QD_even_a();
    }

    action update_QD_odd_a()
    {   
        operate_QD_odd_reg.execute(meta.update_index);
    }
    @stage(4)  table update_QD_odd_t
    {   
        actions={update_QD_odd_a;}
        default_action=update_QD_odd_a();
    }

    action read_QD_even_a()
    {   
        meta.past_QD = operate_QD_even_reg.execute(meta.read_index);
    }
    @stage(4)  table read_QD_even_t
    {   
        actions={read_QD_even_a;}
        default_action=read_QD_even_a();
    }

    action read_QD_odd_a()
    {   
        meta.past_QD = operate_QD_odd_reg.execute(meta.read_index);
    }
    @stage(4)  table read_QD_odd_t
    {   
        actions={read_QD_odd_a;}
        default_action=read_QD_odd_a();
    }
    ...
    RegisterAction<bit<32>, bit<16>, bit<32>>(txBytes_reg) operate_txBytes_reg =
    {
        void apply(inout bit<32> register_data, out bit<32> result) {
            result = register_data;
            register_data = register_data + hdr.bridge.pkt_len;
    ...
    action split_QD_a(bit<4> mantissa, bit<8> exp)
    {   
        meta.QD_mantissa = mantissa;
        meta.QD_exp = exp;
    }
    @stage(5)  table split_QD_t
    {   
        key = {meta.past_QD:ternary;}
        actions={split_QD_a;}
        default_action=split_QD_a(0,0);
    }


    action split_QL_a(bit<4> mantissa, bit<8> exp)
    {   
        meta.QL_mantissa = mantissa;
        meta.QL_exp = exp;
    }
    @stage(5)  table split_QL_t
    {   
        key = {eg_intr_md.deq_qdepth:ternary;}
        actions={split_QL_a;}
        default_action=split_QL_a(0,0);
    }

   


    action cal_step_1_a(bit<4> mantissa_1, bit<4> mantissa_2, bit<8> exp_1, bit<8> exp_2)
    {   
        meta.factor_1_mantissa = mantissa_1; 
        meta.factor_2_mantissa = mantissa_2;
        meta.QL_exp = exp_1 + meta.QL_exp;
        meta.QD_exp = exp_2 + meta.QD_exp;
    }
    @stage(7)  table cal_step_1_t
    {   
        key = {meta.rate_dt:ternary;}
        actions={cal_step_1_a;}
        default_action=cal_step_1_a(0,0,0,0);
    }


    action cal_for_QL_a(bit<4> mantissa, bit<8> exp)
    {   
        meta.QL_mantissa = mantissa;
        meta.QL_exp = exp + meta.QL_exp;

    }
    @stage(8)  table cal_for_QL_t
    {   
        key = {meta.factor_1_mantissa:exact;meta.QL_mantissa:exact;}
        actions={cal_for_QL_a;}
        default_action=cal_for_QL_a(0,0);
    }

    action cal_for_QD_a(bit<4> mantissa, bit<8> exp)
    {   
        meta.QD_mantissa = mantissa;
        meta.QD_exp = exp + meta.QD_exp;

    }
    @stage(8)  table cal_for_QD_t
    {   
        key = {meta.factor_2_mantissa:exact;meta.QD_mantissa:exact;}
        actions={cal_for_QD_a;}
        default_action=cal_for_QD_a(0,0);
    }

    action reform_QL_a(bit<32> QL)
    {   
        meta.QL_mod = QL;

    }
    @stage(9)  table reform_QL_t
    {   
        key = {meta.QL_mantissa:exact;meta.QL_exp:exact;}
        actions={reform_QL_a;}
        default_action=reform_QL_a(0);
    }

    action get_QL_mod_a(bit<32> QD)
    {   
        meta.QL_mod = meta.QL_mod + QD;

    }
    @stage(10)  table get_QL_mod_t
    {   
        key = {meta.QD_mantissa:exact;meta.QD_exp:exact;}
        actions={get_QL_mod_a;}
        default_action=get_QL_mod_a(0);
    }
    apply 
    {
        if (hdr.tcp.isValid())
        {   
            ...
            if (hdr.tcp.flags_ack == 0)
            {
                ...
                {
                    read_QD_odd_t.apply();
                    update_QD_even_t.apply();
                }
                else
                {
                    update_QD_odd_t.apply();
                    read_QD_even_t.apply();
                }
            }
